...
if($_SESSION['groupid']==1){
  ...
  echo "<center>Welcome Doctor</center>";
}else if($_SESSION['groupid']==3){
  echo "<center>Nurse</center>";
}else if($_SESSION['groupid']==2){
  echo "<center>Welcome Patient</center>";
}else if($_SESSION['groupid']==4){
  echo "<center>Welcome Administrator</center>";
}
...
if($total_results = $db->getRow("SELECT COUNT(*) as count FROM `".MLS_PREFIX."users` ?p", $where)->count) {
    ...
    $perpage = 50; // number of elements perpage
    ...
    $start = ($page_number - 1) * $perpage;
     ...
     $wherem = $db->parse("WHERE groupid=2 or groupid=3 and `username` LIKE ?s ", '%'.$_GET['q'].'%');

       
    }else if ($_SESSION['groupid']==2){
   //  echo "patient";
   $wherem = $db->parse("WHERE groupid=5 and `username` LIKE ?s ", '%'.$_GET['q'].'%');

   //echo where;
  }else if ($_SESSION['groupid']==5){
   //  echo "patient";
   $wherem = $db->parse("WHERE groupid=2 and `username` LIKE ?s ", '%'.$_GET['q'].'%');

   //echo where;
  }else if ($_SESSION['groupid']==4){
   //  echo "patient";
   $wherem = $db->parse("WHERE groupid=1 or groupid=2 or groupid=3 or groupid=4 or groupid=5 and `username` LIKE ?s ", '%'.$_GET['q'].'%');

   //echo where;
  }else if ($_SESSION['groupid']==1){
   //  echo "patient";
   $wherem = $db->parse("WHERE groupid=1  and `username` LIKE ?s ", '%'.$_GET['q'].'%');
   ...
   //echo where;
    ...
    $data = $db->getAll("SELECT * FROM `".MLS_PREFIX."users` ?p ORDER BY ?p LIMIT ?i,?i", $wherem, $order_by, $start, $perpage);
    if(!$data){
      echo mysql_error();
    }
